...
app.get('/',function(req, res){
    session
        ...
        .then(function(result){
            ...
            session 
                .run("MATCH (n:Locations) RETURN n")
...
// Rota Adiciona localidade
app.post('/location/add', function(req,res){
    var city = req.body.city;
    var state = req.body.state;
    session 
        .run("CREATE(n:Locations {city:{cityParam}, state:{stateParam}}) RETURN n", {cityParam: city, stateParam: state})
        .then(function(result){
            res.redirect('/');
            session.close();
        })
...
app.post('/person/born/add', function(req,res){
    ...
    var id = req.body.id;
    session 
        .run("MATCH(a:Person {name: {nameParam}}), (b:Locations {city: {cityParam}, state:{stateParam}}) MERGE(a)-[r:NASCEU_EM {year:{yearParam}}]->(b) RETURN a, b", {nameParam: name, cityParam: city, stateParam: state, yearParam: year})
        .then(function(result){
            if(id && id != null){
                res.redirect('/person/'+id);
            }else{
...
app.get('/person/:id', function(req,res){
    ...
    session
        ...
        .then(function(result){
            ...
            session
                .run("OPTIONAL MATCH (a:Person)-[r:NASCEU_EM]-(b:Locations) WHERE id(a)=toInt({idParam}) RETURN b.city as city, b.state as state", {idParam:id})
                .then(function(result2){
